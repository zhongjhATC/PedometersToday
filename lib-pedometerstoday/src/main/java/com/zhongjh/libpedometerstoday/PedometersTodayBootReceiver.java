...
public class PedometersTodayBootReceiver extends BroadcastReceiver {

    @Override
    public void onReceive(Context context, Intent intent) {
        try {
            // 启动服务
            Intent pedometersTodayIntent = new Intent(context, PedometersTodayService.class);
            pedometersTodayIntent.putExtra(PedometersTodayService.INTENT_NAME_BOOT, true);
            ContextCompat.startForegroundService(context, pedometersTodayIntent);
        } catch (Exception e) {
            e.printStackTrace();
