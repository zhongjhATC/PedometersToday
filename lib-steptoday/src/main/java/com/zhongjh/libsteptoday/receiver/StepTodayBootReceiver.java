...
public class StepTodayBootReceiver extends BroadcastReceiver {

    @Override
    public void onReceive(Context context, Intent intent) {
        try {
            // 启动服务
            Intent stepTodayIntent = new Intent(context, StepTodayService.class);
            stepTodayIntent.putExtra(StepTodayService.INTENT_NAME_BOOT, true);
            ContextCompat.startForegroundService(context, stepTodayIntent);
        } catch (Exception e) {
            e.printStackTrace();
